...
export async function service_add(req, res, next) {
	...
	try {
		...
		const buckets_bucketId_newOwnerPublicKey =
			req.body.buckets_bucketId_newOwnerPublicKey || 0;
...
export async function service_remove(req, res, next) {
	...
	try {
		...
		const { toBucketId } = req.body;
		const buckets_fromBucketId_newOwnerPublicKey =
			req.body.buckets_fromBucketId_newOwnerPublicKey || 0;
		const buckets_toBucketId_newOwnerPublicKey =
			req.body.buckets_toBucketId_newOwnerPublicKey || 0;
...
export async function service_remove1(req, res, next) {
	...
	try {
		...
		const { value } = req.body;
		const a_newOwnerPublicKey = req.body.a_newOwnerPublicKey || 0;
		const { tx, encEvent } = await remove1(value, a_newOwnerPublicKey);
		// prints the tx
		console.log(tx);
		res.send({ tx, encEvent });
		// reassigns leafIndex to the index of the first commitment added by this function
