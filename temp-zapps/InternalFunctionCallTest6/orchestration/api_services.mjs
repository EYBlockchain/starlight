...
import remove3 from "./remove3.mjs";
...
export async function service_addB(req, res, next) {
	try {
		...
		const { tx, encEvent } = await addB(value, value1, a_newOwnerPublicKey);
		// prints the tx
		console.log(tx);
...
export async function service_addA(req, res, next) {
	try {
		...
		const { value } = req.body;
		const d_newOwnerPublicKey = req.body.d_newOwnerPublicKey || 0;
		const { tx, encEvent } = await addA(value, d_newOwnerPublicKey);
		// prints the tx
		console.log(tx);
		res.send({ tx, encEvent });
...
// eslint-disable-next-line func-names
export async function service_remove(req, res, next) {
	try {
		// Initialize the contract
		const contract = new Contract("AssignShield");
		await contract.init();
		...
		const a_newOwnerPublicKey = req.body.a_newOwnerPublicKey || 0;
		const d_newOwnerPublicKey = req.body.d_newOwnerPublicKey || 0;
		const { tx, encEvent } = await remove(
			value,
			value1,
			b_newOwnerPublicKey,
			a_newOwnerPublicKey,
...
// eslint-disable-next-line func-names
export async function service_remove1(req, res, next) {
	try {
		// Initialize the contract
		const contract = new Contract("AssignShield");
		await contract.init();
		...
		const a_newOwnerPublicKey = req.body.a_newOwnerPublicKey || 0;
		const b_newOwnerPublicKey = req.body.b_newOwnerPublicKey || 0;
		const { tx, encEvent } = await remove1(
			value,
			value1,
			a_newOwnerPublicKey,
			b_newOwnerPublicKey
...
// eslint-disable-next-line func-names
export async function service_remove2(req, res, next) {
	try {
		// Initialize the contract
		const contract = new Contract("AssignShield");
		await contract.init();
		...
		const b_newOwnerPublicKey = req.body.b_newOwnerPublicKey || 0;
		const d_newOwnerPublicKey = req.body.d_newOwnerPublicKey || 0;
		const { tx, encEvent } = await remove2(
			value,
			value1,
			a_newOwnerPublicKey,
			b_newOwnerPublicKey,
...
// eslint-disable-next-line func-names
export async function service_remove3(req, res, next) {
	try {
		// Initialize the contract
		const contract = new Contract("AssignShield");
		await contract.init();
		...
		const a_newOwnerPublicKey = req.body.a_newOwnerPublicKey || 0;
		const d_newOwnerPublicKey = req.body.d_newOwnerPublicKey || 0;
		const { tx, encEvent } = await remove3(
			value,
			value1,
			a_newOwnerPublicKey,
			d_newOwnerPublicKey
