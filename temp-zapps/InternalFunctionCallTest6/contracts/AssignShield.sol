...
contract AssignShield is MerkleTree {
          ...
          enum FunctionNames { addB, addA, remove, remove1, remove2, remove3 }
        ...
        function verify(
          ...
          if (functionId == uint(FunctionNames.remove1)) {
            uint k = 0;
          ...
          if (functionId == uint(FunctionNames.remove3)) {
            uint k = 0;
...
inputs.customInputs[1] = 1;
        ...
        inputs.latestNullifierRoot = latestNullifierRoot; 
          ...
          bytes4 sig = bytes4(keccak256("addB(uint256,uint256,uint256,uint256,uint256[],uint256,uint256[],uint256[])")) ;  
 	 	 	 if (sig == msg.sig)
...
inputs.customInputs[1] = 1;
      ...
      function remove (uint256 value, uint256 value1, uint256 nullifierRoot, uint256 latestNullifierRoot,uint256[] calldata newNullifiers, uint256 commitmentRoot, uint256[] calldata newCommitments, uint256[] calldata proof) public  {
        ...
        inputs.latestNullifierRoot = latestNullifierRoot; 
          ...
          inputs.newCommitments = newCommitments;
           ...
           verify(proof, uint(FunctionNames.remove), inputs);
	 	 	 	 ...
	 	 	 	 addB (value,value1,nullifierRoot,latestNullifierRoot,newNullifiers,commitmentRoot,newCommitments,proof);
	 	 	 	 addA (value1,nullifierRoot,latestNullifierRoot,newNullifiers,commitmentRoot,newCommitments,proof);

      }


      function remove1 (uint256 value, uint256 value1, uint256 nullifierRoot, uint256 latestNullifierRoot,uint256[] calldata newNullifiers, uint256 commitmentRoot, uint256[] calldata newCommitments, uint256[] calldata proof) public  {
...
inputs.customInputs[1] = 1;
        ...
        inputs.latestNullifierRoot = latestNullifierRoot; 
          ...
          inputs.newCommitments = newCommitments;
           ...
           verify(proof, uint(FunctionNames.remove1), inputs);
...
inputs.customInputs[2] = 1;
      ...
      function remove3 (uint256 value, uint256 value1, uint256 nullifierRoot, uint256 latestNullifierRoot,uint256[] calldata newNullifiers, uint256 commitmentRoot, uint256[] calldata newCommitments, uint256[] calldata proof) public  {
...
inputs.customInputs[2] = 1;
        ...
        inputs.latestNullifierRoot = latestNullifierRoot; 
          ...
          inputs.newCommitments = newCommitments;
           ...
           verify(proof, uint(FunctionNames.remove3), inputs);
	 	 	 	 addB (value,value1,nullifierRoot,latestNullifierRoot,newNullifiers,commitmentRoot,newCommitments,proof);
	 	 	 	 addA (value1,nullifierRoot,latestNullifierRoot,newNullifiers,commitmentRoot,newCommitments,proof);
      }
}