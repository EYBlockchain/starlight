...
export default async function add(
	...
	const publicValue = generalise(_publicValue);
	...
	for (let index = 0; index < 5; index++) {
		a = generalise(
			parseInt(a.integer, 10) + parseInt(value[index].integer, 10)
		);
	...
	const txData = await instance.methods
		.add({ name: "publicValue", isConstantArray: true })
		.encodeABI();
