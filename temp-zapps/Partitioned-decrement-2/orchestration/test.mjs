...
describe("MyContractShield", async function () {
		...
		describe("First call", async function () {
			...
			it("should call setb", async () => {
				try {
					...
					const { tx, encEvent } = await setb(184);
					// prints the tx
					console.log(tx);
	...
	describe("incr", async function () {
		...
		describe("First call", async function () {
			this.timeout(3660000);
			it("should call incr", async () => {
				try {
					// this starts up the merkle tree's event filter
					await startEventFilter("MyContractShield");
					// this calls your function! It returns the tx from the shield contract
					// you can replace the values below - numbers are randomly generated
					const { tx, encEvent } = await incr(152, 165);
					// prints the tx
					console.log(tx);
					// reassigns leafIndex to the index of the first commitment added by this function
					if (tx.event) {
	...
	describe("decr", async function () {
		...
		describe("First call", async function () {
			this.timeout(3660000);
			it("should call decr", async () => {
				try {
					// this starts up the merkle tree's event filter
					await startEventFilter("MyContractShield");
					// this calls your function! It returns the tx from the shield contract
					// you can replace the values below - numbers are randomly generated
					const { tx, encEvent } = await decr(162);
					// prints the tx
					console.log(tx);
					// reassigns leafIndex to the index of the first commitment added by this function
					if (tx.event) {
