...
contract MyContractShield is MerkleTree {
          ...
          enum FunctionNames { incrb, joinCommitments }
      ...
      function incrb (uint256[] calldata newNullifiers, uint256 commitmentRoot, uint256[] calldata newCommitments, uint256[] calldata proof) public  {
          ...
          inputs.customInputs = new uint[](5);
        	inputs.customInputs[0] = uint256(uint160(address(msg.sender)));
inputs.customInputs[1] = x[msg.sender];
inputs.customInputs[2] = x[y];
inputs.customInputs[3] = z;
inputs.customInputs[4] = 1;
          ...
          bytes4 sig = bytes4(keccak256("incrb(,,,uint256[],uint256,uint256[],uint256[])")) ;  
 	 	 	 if (sig == msg.sig)

          verify(proof, uint(FunctionNames.incrb), inputs);
      }
}