...
contract BucketsOfBallsShield is MerkleTree {


          enum FunctionNames { deposit, transfer, joinCommitments }
      ...
      function deposit (uint256 amountDeposit, uint256[] calldata newCommitments, uint256[] calldata proof) public  {
          ...
          inputs.customInputs = new uint[](2);
        	inputs.customInputs[0] = amountDeposit;
inputs.customInputs[1] = 1;

          inputs.newCommitments = newCommitments;

          bytes4 sig = bytes4(keccak256("deposit(uint,uint256[],uint256[])")) ;  
 	 	 	 if (sig == msg.sig)
