...
export async function service_add(req, res, next) {
	...
	try {
		...
		const { j } = req.body;
		const { iterate } = req.body;
		const total_newOwnerPublicKey = req.body.total_newOwnerPublicKey || 0;
		const structs_msgSender_newOwnerPublicKey =
			req.body.structs_msgSender_newOwnerPublicKey || 0;
