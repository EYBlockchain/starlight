...
export default async function add(_y, _z_newOwnerPublicKey = 0) {
	...
	if (parseInt(y.integer, 10) > 5 || parseInt(y.integer, 10) < 1) {
		z = generalise(parseInt(y.integer, 10) + 3);
	}

	z = generalise(z);

	if (parseInt(y.integer, 10) < 3) {
		z = generalise(parseInt(y.integer, 10));
	}
	...
	const txData = await instance.methods.add(y).encodeABI();
