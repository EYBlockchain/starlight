...
export default async function remove2(
	_value,
	_value1,
	...
	// Non-membership witness for Nullifier
	const d_nullifier_NonMembership_witness =
		getnullifierMembershipWitness(d_nullifier);

	const d_nullifierRoot = generalise(d_nullifier_NonMembership_witness.root);
	const d_nullifier_path = generalise(
		d_nullifier_NonMembership_witness.path
	).all;
	...
	// Get the new updated nullifier Paths
	const d_updated_nullifier_NonMembership_witness =
		getupdatedNullifierPaths(d_nullifier);
	...
	const res = await generateProof("remove2", allInputs);
