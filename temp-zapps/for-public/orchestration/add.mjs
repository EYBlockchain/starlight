...
export default async function add(_j, _k_newOwnerPublicKey = 0) {
	...
	const k_nullifier_NonMembership_witness =
		getnullifierMembershipWitness(k_nullifier);

	const k_nullifierRoot = generalise(k_nullifier_NonMembership_witness.root);
	const k_nullifier_path = generalise(
		k_nullifier_NonMembership_witness.path
	).all;

	await temporaryUpdateNullifier(k_nullifier);

	// Get the new updated nullifier Paths
	const k_updated_nullifier_NonMembership_witness =
		getupdatedNullifierPaths(k_nullifier);
