...
import assign3 from "./assign3.mjs";

import assign2 from "./assign2.mjs";
...
describe("MyContractShield", async function () {
		...
		describe("First call", async function () {
			...
			it("should call assign", async () => {
				try {
					...
					const { tx } = await assign(config.web3.options.defaultAccount, 152);
					// prints the tx
					console.log(tx);
		...
		describe("Second Call", async function () {
			...
			it("should call assign again", async () => {
				try {
					...
					const { tx } = await assign(config.web3.options.defaultAccount, 81);
					if (tx.events.NewLeaves) {
						console.log(`Merkle tree event returnValues:`);
						console.log(tx.events.NewLeaves.returnValues);
					}
	...
	// eslint-disable-next-line func-names
	describe("assign2", async function () {
		this.timeout(3660000);
		try {
			await web3.connect();
		} catch (err) {
		...
		describe("First call", async function () {
			this.timeout(3660000);
			it("should call assign2", async () => {
				try {
					// this starts up the merkle tree's event filter
					await startEventFilter("MyContractShield");
					// this calls your function! It returns the tx from the shield contract
					// you can replace the values below - numbers are randomly generated
					const { tx } = await assign2(22);
					// prints the tx
					console.log(tx);
					// reassigns leafIndex to the index of the first commitment added by this function
					if (tx.events.NewLeaves) {
		...
		describe("Second Call", async function () {
			this.timeout(3660000);
			it("should call assign2 again", async () => {
				try {
					// this calls your function a second time for incremental cases
					const { tx } = await assign2(187);
					if (tx.events.NewLeaves) {
						console.log(`Merkle tree event returnValues:`);
						console.log(tx.events.NewLeaves.returnValues);
					}
	...
	// eslint-disable-next-line func-names
	describe("assign3", async function () {
		this.timeout(3660000);
		try {
			await web3.connect();
		} catch (err) {
		...
		describe("First call", async function () {
			this.timeout(3660000);
			it("should call assign3", async () => {
				try {
					// this starts up the merkle tree's event filter
					await startEventFilter("MyContractShield");
					// this calls your function! It returns the tx from the shield contract
					// you can replace the values below - numbers are randomly generated
					const { tx } = await assign3(189);
					// prints the tx
					console.log(tx);
					// reassigns leafIndex to the index of the first commitment added by this function
					if (tx.events.NewLeaves) {
		...
		describe("Second Call", async function () {
			this.timeout(3660000);
			it("should call assign3 again", async () => {
				try {
					// this calls your function a second time for incremental cases
					const { tx } = await assign3(163);
					if (tx.events.NewLeaves) {
						console.log(`Merkle tree event returnValues:`);
						console.log(tx.events.NewLeaves.returnValues);
					}
