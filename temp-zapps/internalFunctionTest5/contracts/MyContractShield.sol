...
contract MyContractShield is MerkleTree {
          ...
          enum FunctionNames { assign, assign2, assign3, joinCommitments }
        ...
        function verify(
          ...
          if (functionId == uint(FunctionNames.assign2)) {
            uint k = 0;
          ...
          if (functionId == uint(FunctionNames.assign3)) {
            uint k = 0;
      ...
      function assign (uint256[] calldata newCommitments, uint256[] calldata proof) public  {
          ...
          bytes4 sig = bytes4(keccak256("assign(uint256[],uint256[])")) ;  
 	 	 	 if (sig == msg.sig)
      ...
      function assign2 (uint256[] calldata newNullifiers, uint256 commitmentRoot, uint256[] calldata newCommitments, uint256[] calldata proof) public  {
          ...
          bytes4 sig = bytes4(keccak256("assign2(uint256[],uint256,uint256[],uint256[])")) ;  
 	 	 	 if (sig == msg.sig)

          verify(proof, uint(FunctionNames.assign2), inputs);
      }


      function assign3 (uint256[] calldata newNullifiers, uint256 commitmentRoot, uint256[] calldata newCommitments, uint256[] calldata proof) public  {
        ...
        require(msg.sender == admin);
          ...
          bytes4 sig = bytes4(keccak256("assign3(uint256[],uint256,uint256[],uint256[])")) ;  
 	 	 	 if (sig == msg.sig)

          verify(proof, uint(FunctionNames.assign3), inputs);
      }
}