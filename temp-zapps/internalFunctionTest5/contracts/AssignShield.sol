...
contract AssignShield is MerkleTree {
      ...
      function addB (uint256 nullifierRoot, uint256 latestNullifierRoot,uint256[] calldata newNullifiers, uint256 commitmentRoot, uint256[] calldata newCommitments, uint256[] calldata proof) public  {
        ...
        inputs.latestNullifierRoot = latestNullifierRoot; 
          ...
          bytes4 sig = bytes4(keccak256("addB(uint256,uint256,uint256[],uint256,uint256[],uint256[])")) ;  
 	 	 	 if (sig == msg.sig)
        ...
        inputs.latestNullifierRoot = latestNullifierRoot; 
          ...
          inputs.newCommitments = newCommitments;
           ...
           verify(proof, uint(FunctionNames.remove), inputs);
	 	 	 	 addB (nullifierRoot,latestNullifierRoot,newNullifiers,commitmentRoot,newCommitments,proof);
