...
import terminateContract from "./terminateContract.mjs";

import addPublic from "./addPublic.mjs";
...
export async function service_add(req, res, next) {
	...
	try {
		...
		const { tx, encEvent } = await add(value, a_newOwnerPublicKey);
		// prints the tx
		console.log(tx);
...
// eslint-disable-next-line func-names
export async function service_addPublic(req, res, next) {
	try {
		await web3.connect();
		await new Promise((resolve) => setTimeout(() => resolve(), 3000));
	} catch (err) {
	...
	const { value } = req.body;
	const { tx } = await addPublic(value);
	// prints the tx
	console.log(tx);
	res.send({ tx });
	// reassigns leafIndex to the index of the first commitment added by this function
...
export async function service_terminateContract(req, res, next) {
	...
	const { tx } = await terminateContract();
	// prints the tx
	console.log(tx);
	res.send({ tx });
	// reassigns leafIndex to the index of the first commitment added by this function
