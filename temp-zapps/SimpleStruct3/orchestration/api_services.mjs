...
export async function service_add(req, res, next) {
	try {
		...
		const myrct = { amount: req.body.myrct.amount, tax: req.body.myrct.tax };
		const total_msgSender_newOwnerPublicKey =
			req.body.total_msgSender_newOwnerPublicKey || 0;
		const cmt_newOwnerPublicKey = req.body.cmt_newOwnerPublicKey || 0;
...
export async function service_remove(req, res, next) {
	try {
		...
		const invalidrct = {
			amount: req.body.invalidrct.amount,
			tax: req.body.invalidrct.tax,
		};
		const total_msgSender_newOwnerPublicKey =
			req.body.total_msgSender_newOwnerPublicKey || 0;
		const cmt_newOwnerPublicKey = req.body.cmt_newOwnerPublicKey || 0;
