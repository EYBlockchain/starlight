...
class Contract {
	...
	async init() {
		...
		if (!this.contractInterface || !this.contractInterface.abi) {
			throw new Error(
				`Failed to load contract interface for ${this.contractName}`
			);
		}
		...
		if (!this.deployedAddress) {
			throw new Error(
				`Failed to get deployed address for ${this.contractName}`
			);
		}
		...
		this.contractInstance = new web3.eth.Contract(
			this.contractInterface.abi,
			this.deployedAddress,
			config.web3.options
		);
		logger.info(
			`Contract instance created for ${this.contractName} at ${this.deployedAddress}`
		);
	}
	...
	async getContractAddress() {
		...
		if (!deployedAddress) {
			throw new Error(
				"Failed to retrieve contract address after multiple attempts"
			);
		}
	...
	async deploy(userAddress, userAddressPassword, constructorParams) {
		...
		const deployedContractAddress = await this.contractInstance
			...
			.then((deployedContractInstance) => {
				logger.info(
					`${this.contractName} contract deployed at address ${deployedContractInstance.options.address}`
				);
	...
	async registerKey(_secretKey, registerWithContract) {
		...
		if (registerWithContract) {
			...
			const txData = await this.contractInstance.methods
			...
			const sendTxn = await web3.eth.sendSignedTransaction(
