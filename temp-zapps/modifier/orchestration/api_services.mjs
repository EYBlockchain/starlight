...
export async function OwnerShield() {
	const web3 = web3Instance.getConnection();
}
// eslint-disable-next-line func-names
export async function service_alpha(req, res, next) {
	try {
		...
		await addConstructorNullifiers();
		const { tx, encEvent } = await alpha(a_newOwnerPublicKey);
		// prints the tx
		console.log(tx);
...
// eslint-disable-next-line func-names
export async function service_beta(req, res, next) {
	try {
		// Initialize the contract
		const contract = new Contract("OwnerShield");
		await contract.init();
		...
		const a_newOwnerPublicKey = req.body.a_newOwnerPublicKey || 0;
		await addConstructorNullifiers();
		const { tx, encEvent } = await beta(a_newOwnerPublicKey);
		// prints the tx
		console.log(tx);
		res.send({ tx, encEvent });
...
// eslint-disable-next-line func-names
export async function service_gamma(req, res, next) {
	try {
		// Initialize the contract
		const contract = new Contract("OwnerShield");
		await contract.init();
		...
		const a_newOwnerPublicKey = req.body.a_newOwnerPublicKey || 0;
		await addConstructorNullifiers();
		const { tx, encEvent } = await gamma(a_newOwnerPublicKey);
		// prints the tx
		console.log(tx);
		res.send({ tx, encEvent });
