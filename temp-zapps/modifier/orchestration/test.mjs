...
describe("OwnerShield", async function () {
	const web3 = web3Instance.getConnection();
	// eslint-disable-next-line func-names
	describe("alpha", async function () {
		...
		describe("First call", async function () {
			this.timeout(3660000);
			it("should call alpha", async () => {
				try {
					// this starts up the merkle tree's event filter
					await startEventFilter("OwnerShield");
			...
			it("should update the merkle tree", async () => {
				try {
					...
					const path = await getSiblingPath("OwnerShield", leafIndex);
	...
	describe("beta", async function () {
		...
		describe("First call", async function () {
			this.timeout(3660000);
			it("should call beta", async () => {
				try {
					// this starts up the merkle tree's event filter
					await startEventFilter("OwnerShield");
					// this calls your function! It returns the tx from the shield contract
					// you can replace the values below - numbers are randomly generated
					const { tx, encEvent } = await beta();
					// prints the tx
					console.log(tx);
					// reassigns leafIndex to the index of the first commitment added by this function
					if (tx.event) {
		...
		describe("Second Call", async function () {
			this.timeout(3660000);
			it("should call beta again", async () => {
				try {
					// this calls your function a second time for incremental cases
					const { tx } = await beta();
					if (tx.event) {
	...
	describe("gamma", async function () {
		...
		describe("First call", async function () {
			this.timeout(3660000);
			it("should call gamma", async () => {
				try {
					// this starts up the merkle tree's event filter
					await startEventFilter("OwnerShield");
					// this calls your function! It returns the tx from the shield contract
					// you can replace the values below - numbers are randomly generated
					const { tx, encEvent } = await gamma();
					// prints the tx
					console.log(tx);
					// reassigns leafIndex to the index of the first commitment added by this function
					if (tx.event) {
		...
		describe("Second Call", async function () {
			this.timeout(3660000);
			it("should call gamma again", async () => {
				try {
					// this calls your function a second time for incremental cases
					const { tx } = await gamma();
					if (tx.event) {
						console.log(`Merkle tree event returnValues:`);
						console.log(tx.returnValues[0]);
					}
