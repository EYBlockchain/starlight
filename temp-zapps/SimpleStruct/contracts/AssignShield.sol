...
struct MyStruct {
      ...
      function add (uint256 nullifierRoot, uint256 latestNullifierRoot,uint256[] calldata newNullifiers, uint256 commitmentRoot, uint256[] calldata newCommitments, uint256[] calldata proof) public  {
        ...
        inputs.latestNullifierRoot = latestNullifierRoot; 
          ...
          bytes4 sig = bytes4(keccak256("add(uint256,uint256,uint256[],uint256,uint256[],uint256[])")) ;  
 	 	 	 if (sig == msg.sig)
        ...
        inputs.latestNullifierRoot = latestNullifierRoot; 
          ...
          inputs.newCommitments = newCommitments;
           ...
           verify(proof, uint(FunctionNames.remove), inputs);
	 	 	 	 add (nullifierRoot,latestNullifierRoot,newNullifiers,commitmentRoot,newCommitments,proof);
