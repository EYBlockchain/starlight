...
contract AssignShield is MerkleTree {
      ...
      function add (uint256 nullifierRoot, uint256 latestNullifierRoot,uint256[] calldata newNullifiers, uint256 commitmentRoot, uint256[] calldata newCommitments, uint256[] calldata proof) public payable {

        
          Inputs memory inputs;

          inputs.customInputs = new uint[](3);
        	inputs.customInputs[0] = msg.value;
inputs.customInputs[1] = uint256(uint160(address(msg.sender)));
