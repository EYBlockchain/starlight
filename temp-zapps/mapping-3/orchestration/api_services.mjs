...
export async function service_assign(req, res, next) {
	...
	try {
		...
		const a_param1_newOwnerPublicKey = req.body.a_param1_newOwnerPublicKey || 0;
...
// eslint-disable-next-line func-names
export async function service_assign2(req, res, next) {
	try {
		await web3.connect();
		await new Promise((resolve) => setTimeout(() => resolve(), 3000));
	} catch (err) {
	...
	try {
		...
		const a_msgSender_newOwnerPublicKey =
			req.body.a_msgSender_newOwnerPublicKey || 0;
		const b_newOwnerPublicKey = req.body.b_newOwnerPublicKey || 0;
		const { tx, encEvent } = await assign2(
			param3,
...
// eslint-disable-next-line func-names
export async function service_assign3(req, res, next) {
	try {
		await web3.connect();
		await new Promise((resolve) => setTimeout(() => resolve(), 3000));
	} catch (err) {
	...
	try {
		await startEventFilter("MyContractShield");
		const { param5 } = req.body;
		const b_newOwnerPublicKey = req.body.b_newOwnerPublicKey || 0;
		const { tx, encEvent } = await assign3(param5, b_newOwnerPublicKey);
		// prints the tx
		console.log(tx);
		res.send({ tx, encEvent });
		// reassigns leafIndex to the index of the first commitment added by this function
