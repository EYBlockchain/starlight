...
export default async function decra(
	_param2,
	...
	const a_newCommitmentValue = generalise(parseInt(param2.integer, 10));
	// First check if required commitments exist or not

	let [a_commitmentFlag, a_0_oldCommitment, a_1_oldCommitment] =
		getInputCommitments(
			publicKey.hex(32),
	...
	if (a_1_oldCommitment === null && a_commitmentFlag) {
		...
		[a_commitmentFlag, a_0_oldCommitment, a_1_oldCommitment] =
			getInputCommitments(
				publicKey.hex(32),
				a_newCommitmentValue.integer,
				a_preimage
	...
	const a_0_nullifier_NonMembership_witness =
		getnullifierMembershipWitness(a_0_nullifier);
	const a_1_nullifier_NonMembership_witness =
		getnullifierMembershipWitness(a_1_nullifier);
	...
	const a_0_updated_nullifier_NonMembership_witness =
		getupdatedNullifierPaths(a_0_nullifier);
	const a_1_updated_nullifier_NonMembership_witness =
		getupdatedNullifierPaths(a_1_nullifier);
	...
	const res = await generateProof("decra", allInputs);
