...
import incra from "./incra.mjs";

import decrb from "./decrb.mjs";

import decra from "./decra.mjs";
...
describe("MyContractShield", async function () {
		...
		describe("First call", async function () {
			...
			it("should call assign", async () => {
				try {
					...
					const { tx, encEvent } = await assign(91, 70, 119);
					// prints the tx
					console.log(tx);
			...
			it("should update the merkle tree", async () => {
				try {
					...
					const path = await getSiblingPath("MyContractShield", leafIndex);
		...
		describe("Second Call", async function () {
			...
			it("should call assign again", async () => {
				try {
					...
					const { tx } = await assign(151, 182, 170);
					if (tx.event) {
						console.log(`Merkle tree event returnValues:`);
						console.log(tx.returnValues[0]);
					}
	...
	describe("decra", async function () {
		...
		describe("First call", async function () {
			this.timeout(3660000);
			it("should call decra", async () => {
				try {
					// this starts up the merkle tree's event filter
					await startEventFilter("MyContractShield");
					// this calls your function! It returns the tx from the shield contract
					// you can replace the values below - numbers are randomly generated
					const { tx, encEvent } = await decra(58);
					// prints the tx
					console.log(tx);
					// reassigns leafIndex to the index of the first commitment added by this function
					if (tx.event) {
	...
	describe("decrb", async function () {
		...
		describe("First call", async function () {
			this.timeout(3660000);
			it("should call decrb", async () => {
				try {
					// this starts up the merkle tree's event filter
					await startEventFilter("MyContractShield");
					// this calls your function! It returns the tx from the shield contract
					// you can replace the values below - numbers are randomly generated
					const { tx, encEvent } = await decrb(107, 181);
					// prints the tx
					console.log(tx);
					// reassigns leafIndex to the index of the first commitment added by this function
					if (tx.event) {
	...
	describe("incra", async function () {
		...
		describe("First call", async function () {
			this.timeout(3660000);
			it("should call incra", async () => {
				try {
					// this starts up the merkle tree's event filter
					await startEventFilter("MyContractShield");
					// this calls your function! It returns the tx from the shield contract
					// you can replace the values below - numbers are randomly generated
					const { tx, encEvent } = await incra(155);
					// prints the tx
					console.log(tx);
					// reassigns leafIndex to the index of the first commitment added by this function
					if (tx.event) {
		...
		describe("Second Call", async function () {
			this.timeout(3660000);
			it("should call incra again", async () => {
				try {
					// this calls your function a second time for incremental cases
					const { tx } = await incra(97);
					if (tx.event) {
						console.log(`Merkle tree event returnValues:`);
						console.log(tx.returnValues[0]);
					}
