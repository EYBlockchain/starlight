...
contract MyContractShield is MerkleTree {
          ...
          enum FunctionNames { setb, incr, decr, joinCommitments }
        ...
        function verify(
          ...
          if (functionId == uint(FunctionNames.setb)) {
            uint k = 0;
          ...
          if (functionId == uint(FunctionNames.incr)) {
            uint k = 0;
          ...
          if (functionId == uint(FunctionNames.decr)) {
            uint k = 0;
      ...
      function setb (uint256[] calldata newNullifiers, uint256 commitmentRoot, uint256[] calldata newCommitments, uint256[] calldata proof) public  {
          ...
          bytes4 sig = bytes4(keccak256("setb(uint256[],uint256,uint256[],uint256[])")) ;  
 	 	 	 if (sig == msg.sig)

          verify(proof, uint(FunctionNames.setb), inputs);
      }


      function incr (uint256[] calldata newCommitments, uint256[] calldata proof) public  {
          ...
          bytes4 sig = bytes4(keccak256("incr(uint256[],uint256[])")) ;  
 	 	 	 if (sig == msg.sig)

          verify(proof, uint(FunctionNames.incr), inputs);
      }


      function decr (uint256[] calldata newNullifiers, uint256 commitmentRoot, uint256[] calldata newCommitments, uint256[] calldata checkNullifiers, uint256[] calldata proof) public  {
          ...
          bytes4 sig = bytes4(keccak256("decr(uint256[],uint256,uint256[],uint256[],uint256[])")) ;  
 	 	 	 if (sig == msg.sig)

          verify(proof, uint(FunctionNames.decr), inputs);
      }
}