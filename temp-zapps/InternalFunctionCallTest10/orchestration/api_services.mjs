...
// eslint-disable-next-line func-names
export async function service_addC(req, res, next) {
	try {
		// Initialize the contract
		const contract = new Contract("AssignShield");
		await contract.init();
		...
		const { value1 } = req.body;
		const d_newOwnerPublicKey = req.body.d_newOwnerPublicKey || 0;
		const { tx, encEvent } = await addC(value, value1, d_newOwnerPublicKey);
		// prints the tx
		console.log(tx);
		res.send({ tx, encEvent });
...
// eslint-disable-next-line func-names
export async function service_remove4(req, res, next) {
	try {
		// Initialize the contract
		const contract = new Contract("AssignShield");
		await contract.init();
		...
		const { value1 } = req.body;
		const d_newOwnerPublicKey = req.body.d_newOwnerPublicKey || 0;
		const { tx, encEvent } = await remove4(value, value1, d_newOwnerPublicKey);
		// prints the tx
		console.log(tx);
		res.send({ tx, encEvent });
