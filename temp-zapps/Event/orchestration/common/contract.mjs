...
export async function registerKey(
	...
	const publicKey = generalise(
		BigInt(utils.shaHash(secretKey.hex(32))) % BN128_GROUP_ORDER
	);
	...
	if (isStruct) {
		...
		value.forEach((propValue, i) => {
			possibleCommitments.sort(
				(preimageA, preimageB) =>
					parseInt(Object.values(preimageB[1].value)[i], 10) -
					parseInt(Object.values(preimageA[1].value)[i], 10)
			);
			if (
	...
	for (var i = 0; i < possibleCommitments.length; i++) {
		for (var j = 0; j < possibleCommitments.length; j++) {
			if (possibleCommitments[i][j] && possibleCommitments[i][j].value)
				commitmentsSum =
					commitmentsSum + parseInt(possibleCommitments[i][j].value, 10);
		}
	}
	if (
...
export async function joinCommitments(
	...
	const oldCommitment_0_prevSalt = generalise(
		commitments[oldCommitment_0].salt
	);
	const oldCommitment_1_prevSalt = generalise(
		commitments[oldCommitment_1].salt
	);
