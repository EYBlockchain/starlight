...
export default async function alpha(
	...
	if (!fs.existsSync(keyDb))
		await contract.registerKey(utils.randomHex(31), "OwnerShield", true);
	...
	const gm_account_nullifier_NonMembership_witness = getnullifierMembershipWitness(
		gm_account_nullifier
	);
	...
	// Non-membership witness for Nullifier
	const admin12_nullifier_NonMembership_witness = getnullifierMembershipWitness(
		admin12_nullifier
	);
	...
	const gm_account_updated_nullifier_NonMembership_witness = getupdatedNullifierPaths(
		gm_account_nullifier
	);
	...
	// Get the new updated nullifier Paths
	const admin12_updated_nullifier_NonMembership_witness = getupdatedNullifierPaths(
		admin12_nullifier
	);
