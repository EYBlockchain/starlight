...
import decrement from "./decrement.mjs";
...
// eslint-disable-next-line func-names
export async function service_increment(req, res, next) {
	try {
		...
		const { tx, encEvent } = await increment(
			param2,
...
// eslint-disable-next-line func-names
export async function service_decrement(req, res, next) {
	try {
		// Initialize the contract
		const contract = new Contract("MyContractShield");
		await contract.init();
		...
		const a_addr_newOwnerPublicKey = req.body.a_addr_newOwnerPublicKey || 0;
		const { tx, encEvent } = await decrement(
			addr,
			param3,
