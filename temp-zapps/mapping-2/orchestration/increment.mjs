...
export default async function increment(
	...
	const allInputs = [
		...
		[
			decompressStarlightKey(a_msgSender_newOwnerPublicKey)[0].integer,
			decompressStarlightKey(a_msgSender_newOwnerPublicKey)[1].integer,
		],
	].flat(Infinity);
	...
	const proof = generalise(Object.values(res.proof).flat(Infinity))
		...
		.flat(Infinity);
	const a_msgSender_cipherText = res.inputs
		.slice(-5, -2)
		.map((e) => generalise(e).integer);
	const a_msgSender_encKey = res.inputs
		.slice(-2)
		.map((e) => generalise(e).integer);
