...
contract MyContractShield is MerkleTree {
          ...
          enum FunctionNames { increment, decrement, joinCommitments, splitCommitments }
        ...
        function verify(
          ...
          if (functionId == uint(FunctionNames.increment)) {
            uint k = 0;
          ...
          if (functionId == uint(FunctionNames.decrement)) {
            uint k = 0;
...
admin = msg.sender;
      ...
      function increment (uint256[] calldata newCommitments, uint256[][] calldata cipherText, uint256[2][] calldata ephPubKeys, uint256[] calldata proof) public  {
          ...
          inputs.encKeys = ephPubKeys;
           ...
           verify(proof, uint(FunctionNames.increment), inputs);
      ...
      function decrement (uint256 nullifierRoot, uint256 latestNullifierRoot,uint256[] calldata newNullifiers, uint256 commitmentRoot, uint256[] calldata newCommitments, uint256[] calldata proof) public  {
        ...
        inputs.latestNullifierRoot = latestNullifierRoot; 
          ...
          inputs.newCommitments = newCommitments;
           ...
           verify(proof, uint(FunctionNames.decrement), inputs);
