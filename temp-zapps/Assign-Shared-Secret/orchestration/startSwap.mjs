...
export default async function startSwap(
	...
	if (
		...
		const tx = await splitCommitments(
			...
			secretKey,
			publicKey,
			[5, balances_msgSender_stateVarId_key],
			balances_msgSender_0_oldCommitment,
	...
	let tokenOwners_tokenIdSent = generalise(sharedAddress.integer);

	tokenOwners_tokenIdSent = generalise(tokenOwners_tokenIdSent);

	swapProposals_sharedAddress.swapTokenSent = generalise(
	...
	swapProposals_sharedAddress.swapTokenRecieved = generalise(
		parseInt(tokenIdRecieved.integer, 10)
	);

	swapProposals_sharedAddress = generalise(swapProposals_sharedAddress);

	pendingStatus = generalise(1);
