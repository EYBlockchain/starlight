...
export default async function completeSwap(
	...
	_tokenOwners_tokenIdRecieved_newOwnerPublicKey = 0,
	...
	let tokenOwners_tokenIdRecieved_newOwnerPublicKey = generalise(
		_tokenOwners_tokenIdRecieved_newOwnerPublicKey
	);
	...
	let tokenOwners_tokenIdRecieved_stateVarId = 9;

	const tokenOwners_tokenIdRecieved_stateVarId_key = tokenIdRecieved;

	tokenOwners_tokenIdRecieved_stateVarId = generalise(
		utils.mimcHash(
			[
				generalise(tokenOwners_tokenIdRecieved_stateVarId).bigInt,
				tokenOwners_tokenIdRecieved_stateVarId_key.bigInt,
			],
			"ALT_BN_254"
		)
	).hex(32);

	let tokenOwners_tokenIdRecieved_commitmentExists = true;
	let tokenOwners_tokenIdRecieved_witnessRequired = true;

	const tokenOwners_tokenIdRecieved_commitment = await getCurrentWholeCommitment(
		tokenOwners_tokenIdRecieved_stateVarId
	);

	let tokenOwners_tokenIdRecieved_preimage = {
		value: 0,
		salt: 0,
		commitment: 0,
	};
	if (!tokenOwners_tokenIdRecieved_commitment) {
		tokenOwners_tokenIdRecieved_commitmentExists = false;
		tokenOwners_tokenIdRecieved_witnessRequired = false;
	} else {
		tokenOwners_tokenIdRecieved_preimage =
			tokenOwners_tokenIdRecieved_commitment.preimage;
	}
	...
	const balances_msgSender_newCommitmentValue = generalise(
		parseInt(amountRecieved.integer, 10) - parseInt(amountSent.integer, 10)
	);
	...
	let balances_counterParty_stateVarId = 5;
	...
	const tokenOwners_tokenIdSent_currentCommitment = tokenOwners_tokenIdSent_commitmentExists
	...
	const tokenOwners_tokenIdRecieved_currentCommitment = tokenOwners_tokenIdRecieved_commitmentExists
		? generalise(tokenOwners_tokenIdRecieved_commitment._id)
		: generalise(0);
	const tokenOwners_tokenIdRecieved_prev = generalise(
		tokenOwners_tokenIdRecieved_preimage.value
	);
	const tokenOwners_tokenIdRecieved_prevSalt = generalise(
		tokenOwners_tokenIdRecieved_preimage.salt
	);
	...
	const tokenOwners_tokenIdSent_witness = tokenOwners_tokenIdSent_witnessRequired
	...
	// generate witness for whole state
	const tokenOwners_tokenIdRecieved_emptyPath = new Array(32).fill(0);
	const tokenOwners_tokenIdRecieved_witness = tokenOwners_tokenIdRecieved_witnessRequired
		? await getMembershipWitness(
				"SwapShield",
				tokenOwners_tokenIdRecieved_currentCommitment.integer
		  )
		: {
				index: 0,
				path: tokenOwners_tokenIdRecieved_emptyPath,
				root: (await getRoot("SwapShield")) || 0,
		  };
	const tokenOwners_tokenIdRecieved_index = generalise(
		tokenOwners_tokenIdRecieved_witness.index
	);
	const tokenOwners_tokenIdRecieved_root = generalise(
		tokenOwners_tokenIdRecieved_witness.root
	);
	const tokenOwners_tokenIdRecieved_path = generalise(
		tokenOwners_tokenIdRecieved_witness.path
	).all;
	...
	swapProposals_sharedAddress.swapAmountRecieved = generalise(
	...
	let tokenOwners_tokenIdSent = generalise(counterParty.integer);

	tokenOwners_tokenIdSent = generalise(tokenOwners_tokenIdSent);

	let tokenOwners_tokenIdRecieved = generalise(msgSender.integer);

	tokenOwners_tokenIdRecieved = generalise(tokenOwners_tokenIdRecieved);
	...
	let tokenOwners_tokenIdSent_nullifier = tokenOwners_tokenIdSent_commitmentExists
		? poseidonHash([
	...
	const tokenOwners_tokenIdSent_nullifier_NonMembership_witness = getnullifierMembershipWitness(
		tokenOwners_tokenIdSent_nullifier
	);
	...
	let tokenOwners_tokenIdRecieved_nullifier = tokenOwners_tokenIdRecieved_commitmentExists
		? poseidonHash([
				BigInt(tokenOwners_tokenIdRecieved_stateVarId),
				BigInt(secretKey.hex(32)),
				BigInt(tokenOwners_tokenIdRecieved_prevSalt.hex(32)),
		  ])
		: poseidonHash([
				BigInt(tokenOwners_tokenIdRecieved_stateVarId),
				BigInt(generalise(0).hex(32)),
	...
	tokenOwners_tokenIdRecieved_nullifier = generalise(
		tokenOwners_tokenIdRecieved_nullifier.hex(32)
	); // truncate
	// Non-membership witness for Nullifier
	const tokenOwners_tokenIdRecieved_nullifier_NonMembership_witness = getnullifierMembershipWitness(
		tokenOwners_tokenIdRecieved_nullifier
	);

	const tokenOwners_tokenIdRecieved_nullifierRoot = generalise(
		tokenOwners_tokenIdRecieved_nullifier_NonMembership_witness.root
	);
	const tokenOwners_tokenIdRecieved_nullifier_path = generalise(
		tokenOwners_tokenIdRecieved_nullifier_NonMembership_witness.path
	).all;
	...
	// Non-membership witness for Nullifier
	const pendingStatus_nullifier_NonMembership_witness = getnullifierMembershipWitness(
		pendingStatus_nullifier
	);
	...
	// Non-membership witness for Nullifier
	const swapProposals_sharedAddress_nullifier_NonMembership_witness = getnullifierMembershipWitness(
		swapProposals_sharedAddress_nullifier
	);
	...
	await temporaryUpdateNullifier(tokenOwners_tokenIdRecieved_nullifier);
	...
	const tokenOwners_tokenIdSent_updated_nullifier_NonMembership_witness = getupdatedNullifierPaths(
		tokenOwners_tokenIdSent_nullifier
	);
	...
	// Get the new updated nullifier Paths
	const tokenOwners_tokenIdRecieved_updated_nullifier_NonMembership_witness = getupdatedNullifierPaths(
		tokenOwners_tokenIdRecieved_nullifier
	);
	const tokenOwners_tokenIdRecieved_nullifier_updatedpath = generalise(
		tokenOwners_tokenIdRecieved_updated_nullifier_NonMembership_witness.path
	).all;
	const tokenOwners_tokenIdRecieved_newNullifierRoot = generalise(
		tokenOwners_tokenIdRecieved_updated_nullifier_NonMembership_witness.root
	);

	// Get the new updated nullifier Paths
	const pendingStatus_updated_nullifier_NonMembership_witness = getupdatedNullifierPaths(
		pendingStatus_nullifier
	);
	...
	// Get the new updated nullifier Paths
	const swapProposals_sharedAddress_updated_nullifier_NonMembership_witness = getupdatedNullifierPaths(
		swapProposals_sharedAddress_nullifier
	);
	...
	const tokenOwners_tokenIdRecieved_newSalt = generalise(utils.randomHex(31));

	let tokenOwners_tokenIdRecieved_newCommitment = poseidonHash([
		BigInt(tokenOwners_tokenIdRecieved_stateVarId),
		BigInt(tokenOwners_tokenIdRecieved.hex(32)),
		BigInt(tokenOwners_tokenIdRecieved_newOwnerPublicKey.hex(32)),
		BigInt(tokenOwners_tokenIdRecieved_newSalt.hex(32)),
	]);

	tokenOwners_tokenIdRecieved_newCommitment = generalise(
		tokenOwners_tokenIdRecieved_newCommitment.hex(32)
	); // truncate
	...
	swapProposals_sharedAddress.swapTokenSent = swapProposals_sharedAddress.swapTokenSent
		? swapProposals_sharedAddress.swapTokenSent
		: swapProposals_sharedAddress_prev.swapTokenSent;
	swapProposals_sharedAddress.swapTokenRecieved = swapProposals_sharedAddress.swapTokenRecieved
	...
	const allInputs = [
		...
		tokenOwners_tokenIdRecieved_commitmentExists
			? secretKey.integer
			: generalise(0).integer,

		tokenOwners_tokenIdRecieved_nullifier.integer,
		tokenOwners_tokenIdRecieved_nullifier_path.integer,
		tokenOwners_tokenIdRecieved_nullifier_updatedpath.integer,
		tokenOwners_tokenIdRecieved_prev.integer,
		tokenOwners_tokenIdRecieved_prevSalt.integer,
		tokenOwners_tokenIdRecieved_commitmentExists ? 0 : 1,

		tokenOwners_tokenIdRecieved_index.integer,
		tokenOwners_tokenIdRecieved_path.integer,
		tokenOwners_tokenIdRecieved_newOwnerPublicKey.integer,
		tokenOwners_tokenIdRecieved_newSalt.integer,
		tokenOwners_tokenIdRecieved_newCommitment.integer,
	...
	if (tokenOwners_tokenIdRecieved_commitmentExists)
		await markNullified(
			tokenOwners_tokenIdRecieved_currentCommitment,
			secretKey.hex(32)
		);
	else await updateNullifierTree(); // Else we always update it in markNullified

	await storeCommitment({
		hash: tokenOwners_tokenIdRecieved_newCommitment,
		name: "tokenOwners",
		mappingKey: tokenOwners_tokenIdRecieved_stateVarId_key.integer,
		preimage: {
			stateVarId: generalise(tokenOwners_tokenIdRecieved_stateVarId),
			value: tokenOwners_tokenIdRecieved,
			salt: tokenOwners_tokenIdRecieved_newSalt,
			publicKey: tokenOwners_tokenIdRecieved_newOwnerPublicKey,
		},
		secretKey:
			tokenOwners_tokenIdRecieved_newOwnerPublicKey.integer ===
			publicKey.integer
