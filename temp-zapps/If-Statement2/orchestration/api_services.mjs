...
// eslint-disable-next-line func-names
export async function service_fn1(req, res, next) {
	try {
...
// eslint-disable-next-line func-names
export async function service_fn2(req, res, next) {
	try {
		// Initialize the contract
		const contract = new Contract("TestShield");
		await contract.init();
		...
		const { p } = req.body;
		const z_newOwnerPublicKey = req.body.z_newOwnerPublicKey || 0;
		const { tx, encEvent } = await fn2(p, z_newOwnerPublicKey);
		// prints the tx
		console.log(tx);
		res.send({ tx, encEvent });
