...
// eslint-disable-next-line func-names
export async function service_setAdmin(req, res, next) {
	try {
		...
		const admin_newOwnerPublicKey = req.body.admin_newOwnerPublicKey || 0;
		await addConstructorNullifiers();
		const { tx, encEvent } = await setAdmin(newAdmin, admin_newOwnerPublicKey);
		// prints the tx
		console.log(tx);
...
export async function service_assign(req, res, next) {
	try {
		...
		await addConstructorNullifiers();
		const { tx, encEvent } = await assign(param1, a_newOwnerPublicKey);
		// prints the tx
		console.log(tx);
		res.send({ tx, encEvent });
