...
// eslint-disable-next-line func-names
export async function service_assignA(req, res, next) {
	try {
		await web3.connect();
	...
	try {
		...
		const { tx, encEvent } = await assignA(a_newOwnerPublicKey);
		// prints the tx
		console.log(tx);
...
// eslint-disable-next-line func-names
export async function service_assignBZ(req, res, next) {
	try {
		await web3.connect();
		await new Promise((resolve) => setTimeout(() => resolve(), 3000));
	} catch (err) {
	...
	try {
		...
		const { tx, encEvent } = await assignBZ(b_newOwnerPublicKey);
		// prints the tx
		console.log(tx);
		res.send({ tx, encEvent });
		// reassigns leafIndex to the index of the first commitment added by this function
