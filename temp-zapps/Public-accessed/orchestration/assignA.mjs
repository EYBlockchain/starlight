...
export default async function assignA(_a_newOwnerPublicKey = 0) {
	...
	const a_stateVarId = generalise(9).hex(32);
	...
	const txData = await instance.methods.assignA().encodeABI();
	...
	const res = await generateProof("assignA", allInputs);
	...
	const txData = await instance.methods
		.assignA([a_newCommitment.integer], proof)
		.encodeABI();
