...
import assignBZ from "./assignBZ.mjs";

import assignA from "./assignA.mjs";
...
describe("MyContractShield", async function () {
	...
	// eslint-disable-next-line func-names
	describe("assignA", async function () {
		...
		describe("First call", async function () {
			...
			it("should call assignA", async () => {
				try {
					...
					const { tx, encEvent } = await assignA();
					// prints the tx
					console.log(tx);
	...
	// eslint-disable-next-line func-names
	describe("assignBZ", async function () {
		this.timeout(3660000);
		try {
			await web3.connect();
		} catch (err) {
		...
		describe("First call", async function () {
			this.timeout(3660000);
			it("should call assignBZ", async () => {
				try {
					// this starts up the merkle tree's event filter
					await startEventFilter("MyContractShield");
					// this calls your function! It returns the tx from the shield contract
					// you can replace the values below - numbers are randomly generated
					const { tx, encEvent } = await assignBZ();
					// prints the tx
					console.log(tx);
					// reassigns leafIndex to the index of the first commitment added by this function
					if (tx.event) {
		...
		describe("Second Call", async function () {
			this.timeout(3660000);
			it("should call assignBZ again", async () => {
				try {
					// this calls your function a second time for incremental cases
					const { tx } = await assignBZ();
					if (tx.event) {
						console.log(`Merkle tree event returnValues:`);
						console.log(tx.returnValues[0]);
					}
