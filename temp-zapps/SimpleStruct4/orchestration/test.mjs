...
describe("ReceiptShield", async function () {
	...
	describe("addPO", async function () {
		...
		describe("Second Call", async function () {
			this.timeout(3660000);
			it("should call addPO again", async () => {
				try {
	...
	// eslint-disable-next-line func-names
	describe("addInvoice", async function () {
		this.timeout(3660000);
		try {
			await web3.connect();
		} catch (err) {
		...
		describe("First call", async function () {
			this.timeout(3660000);
			it("should call addInvoice", async () => {
				try {
					// this starts up the merkle tree's event filter
					await startEventFilter("ReceiptShield");
					// this calls your function! It returns the tx from the shield contract
					// you can replace the values below - numbers are randomly generated
					const { tx, encEvent } = await addInvoice({ amount: 91, id: 183 });
					// prints the tx
					console.log(tx);
					// reassigns leafIndex to the index of the first commitment added by this function
					if (tx.event) {
		...
		describe("Second Call", async function () {
			...
			it("should call addInvoice again", async () => {
				try {
					// this calls your function a second time for incremental cases
					const { tx } = await addInvoice({ amount: 143, id: 91 });
					if (tx.event) {
						console.log(`Merkle tree event returnValues:`);
						console.log(tx.returnValues[0]);
					}
	...
	describe("pay", async function () {
		...
		describe("First call", async function () {
			this.timeout(3660000);
			it("should call pay", async () => {
				try {
					// this starts up the merkle tree's event filter
					await startEventFilter("ReceiptShield");
					// this calls your function! It returns the tx from the shield contract
					// you can replace the values below - numbers are randomly generated
					const { tx, encEvent } = await pay(160, 191);
					// prints the tx
					console.log(tx);
					// reassigns leafIndex to the index of the first commitment added by this function
					if (tx.event) {
