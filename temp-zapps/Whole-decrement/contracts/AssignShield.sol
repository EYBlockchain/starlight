...
inputs.customInputs[2] = newCommitments[0];

          inputs.newCommitments = newCommitments;

          bytes4 sig = bytes4(keccak256("add(uint256,,,uint256[],uint256[])")) ;  
 	 	 	 if (sig == msg.sig)
      ...
      function remove (uint256[] calldata newNullifiers, uint256 commitmentRoot, uint256[] calldata newCommitments, uint256[] calldata proof) public  {
          ...
          inputs.customInputs = new uint[](2);
        	inputs.customInputs[0] = newCommitments[1];
inputs.customInputs[1] = newCommitments[0];
