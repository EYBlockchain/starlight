...
export async function service_add(req, res, next) {
	...
	try {
		...
		const { tx, encEvent, bool } = await add(value, a_newOwnerPublicKey);
		// prints the tx
		console.log(tx);
		res.send({ tx, encEvent, bool });
...
export async function service_remove(req, res, next) {
	...
	try {
		...
		const a_newOwnerPublicKey = req.body.a_newOwnerPublicKey || 0;
		const { tx, encEvent, a_2_newCommitment } = await remove(
			value,
			value1,
			a_newOwnerPublicKey
		);
		// prints the tx
		console.log(tx);
		res.send({ tx, encEvent, a_2_newCommitment });
		// reassigns leafIndex to the index of the first commitment added by this function
		if (tx.event) {
			leafIndex = tx.returnValues[0];
			// prints the new leaves (commitments) added by this function call
