    ...
    case 'IfStatement': {
      if(node.falseBody.statements)
        return `if (${codeGenerator(node.condition)}) {
          ...
          ${codeGenerator(node.falseBody.statements)}
          ${node.falseBody.statements.expression.leftHandSide.name} = generalise(${node.falseBody.statements.expression.leftHandSide.name});
        }`
        else
        return `if (${codeGenerator(node.condition)}) {
          ${codeGenerator(node.trueBody.statements)}
