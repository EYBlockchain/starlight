...
let childOfSecret =  path.getAncestorOfType('ForStatement')?.containsSecret;
      ...
      if (node.expression.nodeType === 'Assignment' || node.expression.nodeType === 'UnaryOperation') {
        ...
        let referencedIndicator = scope.getReferencedIndicator(lhs, true);
        if(!referencedIndicator) referencedIndicator = scope.getReferencedIndicator(lhs.baseExpression, true);
        ...
        if (referencedIndicator?.isMapping && lhs.baseExpression) {
          if(referencedIndicator?.isNestedMapping)
          lhs = lhs.baseExpression.baseExpression;
        else
