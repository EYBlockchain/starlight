...
export default function toContract(ast, options) {
  // transpile to a solidity AST:
  logger.info('Transforming the .zsol AST to a .sol AST...');
  const newAST = transformation1(ast);
  const newASTFilePath = path.join(options.contractsDirPath, `${options.inputFileName}_ast.json`);
  ...
  //
  // logger.info('circuitFileData', circuitFileData)
  //
  ...
  //   const filepath = path.join(options.outputDirPath, fileObj.filepath);
