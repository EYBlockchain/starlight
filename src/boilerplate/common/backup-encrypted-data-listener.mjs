...
// Track in-flight backup processing so other modules can wait for idle
let activeBackupProcesses = 0;

export async function waitForBackupListenerIdle(
  timeoutMs = 100000,
  pollMs = 100,
) {
  const start = Date.now();
  while (activeBackupProcesses > 0) {
    if (Date.now() - start > timeoutMs) {
      throw new Error(
        'Timed out waiting for backup listener to finish processing',
      );
    }
    // eslint-disable-next-line no-await-in-loop
    await new Promise(res => {
      setTimeout(res, pollMs);
    });
  }
}
    ...
    try {
      ...
      this.eventSubscription.on('data', async eventData => {
        try {
          ...
          console.log(
            `[BACKUP] Event received, block ${eventData.blockNumber}`,
          );
      ...
      this.heartbeatInterval = setInterval(async () => {
        ...
        if (!this.eventSubscription || !this.eventSubscription.id) {
          ...
          console.log(
            '[BACKUP] Subscription healthy, ID:',
          ...
          try {
            ...
            if (this.lastProcessedBlock > 0) {
              ...
              if (checkFromBlock <= currentBlock) {
                const pastEvents = await this.instance.getPastEvents(
                  'EncryptedBackupData',
                  {
                ...
                if (pastEvents.length > 0) {
                  ...
                  pastEvents.forEach(evt => {
                    console.log(
                      `[BACKUP]   - Event in block ${evt.blockNumber}, tx: ${evt.transactionHash}`,
                    );
                  });
                  console.log(
                    '[BACKUP] Subscription is broken. Forcing reconnect...',
                  );
                  await this.reconnect();
                } else {
                  console.log(
                    `[BACKUP] No new events from block ${checkFromBlock} to ${currentBlock} - subscription OK`,
                  );
                }
              } else {
                console.log(
                  `[BACKUP] No new blocks since last processed (${this.lastProcessedBlock})`,
                );
              }
            } else {
              console.log(
                '[BACKUP] No events processed yet, skipping past event check',
              );
            }
          } catch (e) {
  ...
  async processBackupEventData(eventData) {
    ...
    try {
      ...
      for (const kp of keyPairs) {
        if (!kp.secretKey) continue; // eslint-disable-line no-continue
        for (let i = 0; i < eventData.returnValues.encPreimages.length; i++) {
          const { cipherText, ephPublicKey } =
          ...
          if (isStruct) {
            ...
            for (let j = start; j < plainText.length; j++) {
              hashInput.push(BigInt(generalise(plainText[j]).hex(32)));
            }
  ...
  async reconnect() {
    ...
    if (this.eventSubscription) {
      try {
        ...
        console.log(
          '[BACKUP] Error unsubscribing (may already be dead):',
          e.message,
        );
      }
