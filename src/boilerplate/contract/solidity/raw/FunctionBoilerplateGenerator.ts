...
class FunctionBoilerplateGenerator {
  ...
  customFunction = {
    parameters({
      ...
      return [
        ...(newNullifiers ? containsAccessedOnlyState ? [`uint256 nullifierRoot, uint256 latestNullifierRoot,uint256[] ${visibility} newNullifiers`] : [`uint256[] ${visibility} newNullifiers`] : []), // nullifiers and nullifier root exist together
    ...
    postStatements({
      ...
      ...(newNullifiers ? containsAccessedOnlyState ?  [`uint256`] : [] : []),
      ...(newNullifiers ? containsAccessedOnlyState ?  [`uint256`] : [] : []),
