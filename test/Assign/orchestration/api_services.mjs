...
import { RemoveManager } from "./remove.mjs";
...
export class ServiceManager {
	constructor(web3) {
		...
		this.remove = new RemoveManager(web3);
	}
	async init() {
		await this.add.init();
		await this.remove.init();
	}
	...
	// eslint-disable-next-line func-names
	async service_remove(req, res, next) {
		try {
			await startEventFilter("AssignShield");
			const { value } = req.body;
			const a_newOwnerPublicKey = req.body.a_newOwnerPublicKey || 0;
			const { tx, encEvent, encBackupEvent } = await this.remove.remove(
				value,
				a_newOwnerPublicKey
			);
			// prints the tx
