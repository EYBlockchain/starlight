...
const collectImportFiles = (file, contextDirPath = boilerplateCircuitsDir) => {
    ...
    const localFilePaths = ImportStatementList.reduce((acc, line) => {
        ...
        if (!(importFilePath.startsWith('utils') ||
...
function codeGenerator(node) {
    ...
    switch (node.nodeType) {
        ...
        case 'ParameterList': {
            ...
            const slicedParamList = paramList.map(p => p.replace('public ', '').replace('private ', ''));
        ...
        case 'VariableDeclaration': {
            const visibility = node.declarationType === 'parameter'
                ? node.isPrivate
                    ? 'private '
                    : 'public '
                : '\t\t';
        ...
        field ${codeGenerator(node.expression)}`;
            ...
            return (_a = codeGenerator(node.expression)) !== null && _a !== void 0 ? _a : '';
        }
        case 'Assignment':
        ...
        default:
            if (!Object.keys(node).length)
                return ''; // we have empty nodes when subnodes are skipped
