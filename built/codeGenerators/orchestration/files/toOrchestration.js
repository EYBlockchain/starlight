...
const collectImportFiles = (file, contextDirPath = boilerplateNodeDir) => {
    ...
    const ImportStatementList = lines.filter(line => (line.includes(`.mjs`) ||
        line.includes(`.json`) ||
...
const editableCommitmentCommonFilesBoilerplate = () => {
    ...
    if (!(importBoilerplate instanceof Array))
        return;
...
const prepareIntegrationTest = (node) => {
    ...
    node.functions.forEach((fn) => {
        ...
        fnboilerplate = fnboilerplate.replace(/FUNCTION_SIG_1/g, paramTypes.map(testInputsByType).join(', '));
        fnboilerplate = fnboilerplate.replace(/FUNCTION_SIG_2/g, paramTypes.map(testInputsByType).join(', '));
        // remove second call
        if (removeSecondCall) {
        ...
        if (removeMerkleTreeTest) {
            ...
            const toRemove = fnboilerplate.match(/it\('should update the merkle tree'?[\s\S]*?}\);/g)[0];
            fnboilerplate = fnboilerplate.replace(toRemove, `\n`);
        }
...
const prepareMigrationsFile = (file, node) => {
    ...
    file.file = file.file.replace(/FUNCTION_NAMES/g, `'${node.functionNames.join(`', '`)}'`);
    // collect any extra constructor parameters
    const constructorParams = ((_a = node.constructorParams) === null || _a === void 0 ? void 0 : _a.map((obj) => obj.name)) || ``;
    ...
    (_b = node.constructorParams) === null || _b === void 0 ? void 0 : _b.forEach((arg) => {
    ...
    if (node.contractImports && constructorParamsIncludesAddr) {
        node.contractImports.forEach((importObj) => {
            ...
            let importedContractName = path.basename(importObj.absolutePath, path.extname(importObj.absolutePath));
            // if import is an interface, we need to deploy contract e.g. IERC20 -> deploy ERC20
            if (importedContractName.startsWith(`I`) &&
                ...
                const newPath = importObj.absolutePath.replace(importedContractName, importedContractName.substring(1));
                ...
                if (check) {
                    ...
                    if (importedContractName === 'ERC20' ||
                        ...
                        logger.warn(`It looks like you're using an ERC contract - please make sure you increase the allowance of the shield contract before testing!`);
    ...
    if ((constructorParams === null || constructorParams === void 0 ? void 0 : constructorParams.length) === 1)
        constructorParams[0] += `,`;
...
export default function fileGenerator(node) {
    ...
    switch (node.nodeType) {
        ...
        case 'File':
            return [
                {
                    filepath: path.join(`./orchestration`, `${node.fileName}${node.fileExtension}`),
        ...
        case 'SetupCommonFilesBoilerplate': {
            ...
            const setupfile = files.filter(obj => obj.filepath.includes(`zkp-setup`))[0];
            const migrationsfile = files.filter(obj => obj.filepath.includes(`shield`))[0];
            // replace placeholder values with ours
            vkfile.file = vkfile.file.replace(/FUNCTION_NAMES/g, `'${node.functionNames.join(`', '`)}'`);
            setupfile.file = setupfile.file.replace(/FUNCTION_NAMES/g, `'${node.functionNames.join(`', '`)}'`);
