        ...
        enter(path) {
            ...
            for (const arg of args) {
                if (arg.nodeType !== 'Identifier')
                    continue;
                if (arg.name === 'this')
                    continue; // you won't find a binding for such a special reference
                ...
                if (node.kind === 'typeConversion' &&
                    ...
                    throw new TODOError(`Type conversions of secret states. We plan to suppport conversions which can be replicated in a zero-knowledge circuit in the near future.`, node);
                if (path.isRequireStatement())
                    throw new TODOError(`Require statements involving secret states. We plan to suppport these (by replicating them with Zokrates assert statements) in the near future.`, node);
                // if (
                //   node.kind === 'functionCall' &&
                //   node.expression.typeDescriptions.typeIdentifier.includes(`_external_`)
                // )
                if (path.isExternalFunctionCall())
                    throw new ZKPError(`We cannot support external function calls with secret arguments - they can't be hidden due to the nature of the blockchain`, node);
                if (node.kind === 'functionCall')
                    throw new TODOError(`Internal function calls involving secret states. We plan to suppport these (by having Zokraes circuits call other circuits) in the near future.`, node);
            }
        },
    },
};
