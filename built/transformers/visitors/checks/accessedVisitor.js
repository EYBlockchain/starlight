...
export default {
    ...
    Identifier: {
        enter(path, state) {
            ...
            const lhsBinding = (lhsNode === null || lhsNode === void 0 ? void 0 : lhsNode.nodeType) === 'Identifier' ||
            ...
            if (!(lhsBinding instanceof VariableBinding))
                return;
            ...
            if (!(referencedBinding === null || referencedBinding === void 0 ? void 0 : referencedBinding.isSecret)) {
                ...
                if (rightAncestor &&
                    ...
                    logger.warn(`Non-secret parameter '${node.name}' used when assigning to a secret variable '${lhsBinding.name}'. Blockchain observers might be able to infer the value of '${lhsBinding.name}' from this. I.e. although you've labelled '${lhsBinding.name}' as 'secret', it might not be secret.`);
            ...
            if (rightAncestor &&
                ...
                if (lhsNode === null)
                    // null is bad
                    throw new TODOError(`We couldn't find a node on the LHS of this RHS node. Please create an issue.`, node);
                if (lhsNode !== false && !path.isInType('FunctionCall')) {
                    ...
                    if (!referencedBinding.stateVariable) {
                        ...
                        if (!lhsBinding.isSecret)
                            ...
                            throw new SyntaxUsageError(`A secret parameter (${node.name}) should not be used to assign to a non-secret variable (${lhsNode.name}). The secret could be deduced by observing how the non-secret variable changes.`, node);
                        if (!lhsBinding.stateVariable)
                            // we have secret param1 = param2 (e.g.) - this is weird
                            logger.warn(`Secret parameter ${node.name} is being used to assign a non-global state. Is this intended?`);
                        return; // we stop, because we never have to access parameters
                    }
                    if (!lhsBinding.isSecret) {
                        // Henceforth `node` must be a stateVariable on the RHS
                        throw new SyntaxUsageError(`Secret state ${node.name} should not be used to assign to a non-secret variable (${lhsNode.name}). The secret could be deduced by observing how the non-secret variable changes.`, node);
                    }
                }
            ...
            if (leftAncestor &&
                ...
                if (!referencedBinding.isWhole &&
                    ...
                    logger.warn(`We don't know whether this state ${node.name} is whole or partitioned - but since there are no known/unknown decorators, we assume its a whole state which nobody but the owner can edit.`);
                logger.debug(`Found an accessed secret state ${node.name} (accessed in ${leftAncestor.parent.operator} operation)`);
                (_b = scope.getReferencedBinding(node)) === null || _b === void 0 ? void 0 : _b.updateAccessed(path);
                const indicator = scope.getReferencedIndicator(node);
                if (indicator instanceof StateVariableIndicator)
                    indicator.updateAccessed(path);
            }
