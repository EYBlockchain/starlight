...
export default {
    SourceUnit: {
        enter(path, state) {
            ...
            if (!contractNames)
                throw new Error('No contracts in AST');
    ...
    ContractDefinition: {
        ...
        exit(path, state) {
            ...
            sourceUnitNodes[contractIndex].baseContracts.push(buildNode('InheritanceSpecifier', {
                nodeType: 'UserDefinedTypeName',
                name: 'MerkleTree',
            }));
            sourceUnitNodes.splice(1, 0, ...buildNode('ContractBoilerplate', {
                bpSection: 'importStatements',
                scope,
            }));
            // unshift in reverse order from how we want them to appear
            contractNodes.unshift(...buildNode('ContractBoilerplate', {
                bpSection: 'verify',
                scope,
            }));
            contractNodes.unshift(...buildNode('ContractBoilerplate', {
    ...
    FunctionDefinition: {
        ...
        exit(path, state) {
            ...
            parameters.push(...buildNode('FunctionBoilerplate', {
                bpSection: 'parameters',
                scope,
            }));
            postStatements.push(...buildNode('FunctionBoilerplate', {
    ...
    Assignment: {
        enter(path, state) {
            ...
            if (binding === null || binding === void 0 ? void 0 : binding.isSecret) {
    ...
    VariableDeclaration: {
        enter(path, state) {
            ...
            const newNode = buildNode('VariableDeclaration', {
                ...
                declarationType,
                typeString: (_a = node.typeDescriptions) === null || _a === void 0 ? void 0 : _a.typeString,
    ...
    Literal: {
        enter(path) {
            ...
            if (!['number', 'bool', 'string'].includes(kind))
                throw new Error(`Only literals of kind "number" or "bool" are currently supported. Found literal of kind ${node.kind}. Please open an issue.`);
            // node._newASTPointer = // no pointer needed, because this is a leaf, so we won't be recursing any further.
        ...
        enter(path, state) {
            ...
            if (path.isRequireStatement()) {
                ...
                const findSecretSubnode = (p, state) => {
                    var _a;
                    const isSecret = (_a = p.getReferencedNode()) === null || _a === void 0 ? void 0 : _a.isSecret;
                    if (isSecret) {
