...
export default {
    VariableDeclarationStatement: {
        enter(node, state) {
            ...
            if (node.declarations[0].nodeType !== 'VariableDeclaration')
                return;
            ...
            for (const toRedecorate of state) {
                ...
                if (toRedecorate.added || toRedecorate.decorator !== 'secret')
                    continue;
                ...
                if (toRedecorate.charStart === Number(srcStart)) {
                    logger.warn(`Superfluous 'secret' decorator used for a local declaration. If the variable interacts with a secret state, it will automatically be kept secret.`);
                    return;
                }
            }
        },
