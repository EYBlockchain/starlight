...
const visitor = {
    ...
    FunctionDefinition: {
        ...
        enter(path, state) {
            ...
            if (scope.modifiesSecretState()) {
                ...
                newImportStatementListNode.imports.push(...buildNode('Boilerplate', {
        ...
        exit(path, state) {
            ...
            newFunctionDefinitionNode.parameters.parameters.push(...buildNode('Boilerplate', {
                bpSection: 'parameters',
                indicators,
            }));
            newFunctionDefinitionNode.body.preStatements.push(...buildNode('Boilerplate', {
                bpSection: 'preStatements',
                indicators,
            }));
            newFunctionDefinitionNode.body.postStatements.push(...buildNode('Boilerplate', {
                bpSection: 'postStatements',
                indicators,
            }));
            if (state.msgSenderParam) {
                node._newASTPointer.parameters.parameters.unshift(buildNode('VariableDeclaration', {
                    name: 'msgSender',
    ...
    VariableDeclarationStatement: {
        enter(path, state) {
            ...
            if (node.stateVariable) {
                throw new Error(`TODO: VariableDeclarationStatements of secret state variables are tricky to initialise because they're assigned-to outside of a function. Future enhancement.`);
            }
            let declarationType;
            if (path.isLocalStackVariableDeclaration())
                declarationType = 'localStack';
            if (path.isFunctionParameterDeclaration())
                declarationType = 'parameter';
            if (declarationType === 'localStack' &&
                !node.isSecret &&
    ...
    Assignment: {
        ...
        exit(path) {
            ...
            const expandAssignment = node => {
                ...
                const expandableOps = ['+=', '-=', '*=', '/=', '%=', '|=', '&=', '^='];
                if (!expandableOps.includes(operator))
                    return node;
    ...
    ExpressionStatement: {
        enter(path, state) {
            ...
            if (isIncremented || isDecremented) {
                switch (expression.nodeType) {
                    case 'Assignment': {
                        ...
                        if (isDecremented) {
                            newNode = buildNode('BoilerplateStatement', Object.assign({ bpType: 'decrementation', indicators: lhsIndicator, subtrahendId: rhs.id }, (lhsIndicator.isMapping && {
                                mappingKeyName: ((_a = lhs.indexExpression) === null || _a === void 0 ? void 0 : _a.name) ||
                            ...
                            // isIncremented
                            newNode = buildNode('BoilerplateStatement', Object.assign({ bpType: 'incrementation', indicators: lhsIndicator, addendId: rhs.id }, (lhsIndicator.isMapping && {
                                mappingKeyName: ((_b = lhs.indexExpression) === null || _b === void 0 ? void 0 : _b.name) ||
                                    lhs.indexExpression.expression.name,
                            })));
                    ...
                    default:
                        throw Error(`Expressions of nodeType ${expression.nodeType} are not yet supported. Please open a new issue in github (if none exists).`);
                }
            }
            ...
            if (node.expression.nodeType === 'Assignment' &&
                ...
                if ((lhs.id === referencedIndicator.referencingPaths[0].node.id ||
                    ...
                    !(referencedIndicator.isWhole &&
                        referencedIndicator.oldCommitmentAccessRequired) // FIX - sometimes a variable will be declared twice when we insert oldCommitmentPreimage preStatements before an overwrite - we check here
                ) {
                    isVarDec = true;
                }
            }
    ...
    VariableDeclaration: {
        enter(path, state) {
            ...
            if (path.isFunctionReturnParameterDeclaration())
                throw new Error(`TODO: VariableDeclarations of return parameters are tricky to initialise because we might rearrange things so they become _input_ parameters to the circuit. Future enhancement.`);
            let declarationType;
    ...
    ElementaryTypeName: {
        enter(path) {
            ...
            if (!supportedTypes.includes(node.name))
                throw new Error(`Currently, only transpilation of types "${supportedTypes}" is supported. Got ${node.name} type.`);
            // node._newASTPointer = // no pointer needed, because this is a leaf, so we won't be recursing any further.
            parent._newASTPointer[path.containerName] = buildNode('ElementaryTypeName', {
                name: node.name === 'bool' ? 'bool' : 'field', // convert uint & address types to 'field', for now.
            });
    ...
    Literal: {
        enter(path) {
            ...
            const { value } = node;
            if (node.kind !== 'number')
                throw new Error(`Only literals of kind "number" are currently supported. Found literal of kind '${node.kind}'. Please open an issue.`);
            // node._newASTPointer = // no pointer needed, because this is a leaf, so we won't be recursing any further.
