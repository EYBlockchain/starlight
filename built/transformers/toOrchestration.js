...
export default function toOrchestration(ast, options) {
    ...
    const newASTFilePath = pathjs.join(options.orchestrationDirPath, `${options.inputFileName}-ast.json`);
    ...
    logger.verbose(`Saving .mjs files to the zApp output directory ${options.orchestrationDirPath}...`);
    for (const fileObj of nodeFileData) {
        ...
        if (!fs.existsSync(dir))
            fs.mkdirSync(dir, { recursive: true }); // required to create the nested folders for common import files.
        if (fileObj.filepath.includes('preimage.json'))
            continue;
    ...
    const contractName = `${contractNode.name.charAt(0).toUpperCase() + contractNode.name.slice(1)}Shield`;
    ...
    if (!(zappFilesBP instanceof Array))
        throw new Error('Boilerplate files not read correctly!');
    let fileObj;
