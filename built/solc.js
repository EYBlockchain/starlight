...
const buildSources = (file, options) => {
    ...
    const rl = file.split('\n');
    if (!options.topDir)
        options.topDir = path.dirname(options.inputFilePath);
    rl.forEach((line) => {
        line = tidy(line);
...
const errorHandling = (compiled) => {
    if (!compiled) {
        ...
        throw new FilingError('Solc Compilation Error: Make sure your .sol contract compiles without Zappify decorators.');
    }
};
/**
