...
import fs from 'fs';
import { traverseNodesFast, traverseNodesFastVisitor, } from '../traverse/traverse.js';
...
function transformation1(oldAST, toRedecorate) {
    ...
    for (const decorator of toRedecorate) {
        ...
        throw new SyntaxError(`We couldn't find where ${decorator.decorator} should be re-added (char number ${decorator.charStart}). Do all decorators precede a variable name?`);
    }
