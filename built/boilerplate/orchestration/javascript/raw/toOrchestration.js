...
export const generateProofBoilerplate = (node) => {
    ...
    for ([stateName, stateNode] of Object.entries(node.privateStates)) {
        ...
        const stateVarIdLines = stateNode.isMapping && !node.parameters.includes(stateNode.stateVarId[1])
            ? [`\n\t\t\t\t\t\t\t\t${stateName}_stateVarId_key.integer,`]
            : [];
        // we add any extra params the circuit needs
        node.parameters
            .filter((para) => !privateStateNames.includes(para) &&
        ...
        switch (stateNode.isWhole) {
            case true:
                output.push(buildBoilerplate('GenerateProof', {
                    stateName,
                    stateType: 'whole',
                ...
                switch (stateNode.nullifierRequired) {
                    case true:
                        ...
                        stateNode.increment.forEach((inc) => {
                            ...
                            if (!output.join().includes(`\t${inc.name}.integer`) &&
...
export const preimageBoilerPlate = (node) => {
    ...
    for ([privateStateName, stateNode] of Object.entries(node.privateStates)) {
        ...
        if (!stateNode.stateVarId[1]) {
            ...
            if (privateStateName.includes(stateNode.stateVarId[1]) &&
        ...
        if (stateNode.accessedOnly) {
            output.push(buildBoilerplate('ReadPreimage', {
                stateType: 'whole',
    ...
    switch (node.nodeType) {
        ...
        case 'InitialisePreimage':
            for ([stateName, stateNode] of Object.entries(node.privateStates)) {
                ...
                lines.push(buildBoilerplate(node.nodeType, {
                    stateName,
                    accessedOnly: stateNode.accessedOnly,
                    mappingKey,
        ...
        case 'SendTransaction':
            ...
            if (params[0][1][0])
                params[0][1] = `${params[0][1]},`; // root - single input
            if (params[0][0][0])
                params[0][0] = `[${params[0][0]}],`; // nullifiers - array
            if (params[0][2][0])
                params[0][2] = `[${params[0][2]}],`; // commitments - array
            if (params[0][3][0])
                params[0][3] = `[${params[0][3]}],`; // accessed nullifiers - array
            return {
                statements: [
