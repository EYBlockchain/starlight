...
export class Binding {
    constructor(path) {
        ...
        const { node: { name, id, nodeType }, node, } = path;
    ...
    isNullifiable() {
        const bindings = Object.entries(this.path.scope.bindings);
...
export class VariableBinding extends Binding {
    constructor(path) {
        ...
        this.isSecret = (_a = node.isSecret) !== null && _a !== void 0 ? _a : false;
    ...
    addMappingKey(referencingPath) {
        ...
        if (keyNode.nodeType !== 'Identifier') {
            throw new TODOError(`A mapping key of nodeType '${keyNode.nodeType}' isn't supported yet. We've only written the code for keys of nodeType Identifier'`, keyNode);
        }
    ...
    updateOwnership(ownerNode, msgIsMappingKeyorMappingValue) {
        var _a, _b;
        ...
        if (this.isOwned &&
            ...
            throw new ZKPError(`We found two distinct owners of a secret state - msg.sender when the mapping key is msg.sender, and when the mapping value is msg.sender, which we can't allow because only one public key needs to be able to open/nullify the secret.`, this.node);
        }
    ...
    prelimTraversalErrorChecks() {
        ...
        if (this.isIncremented &&
            ...
            !this.isKnown &&
            !this.isUnknown) {
            throw new SyntaxUsageError(`Secret state '${this.name}' incremented, but known-ness unknown. Please let us know the known-ness by specifying known/unknown, and if you don't know, let us know.`, this.node);
        }
        ...
        if (this.reinitialisable && this.isPartitioned) {
            throw new SyntaxUsageError(`Can't mark a partitioned state as 'reinitialisable' - partitioned states do not need nullfiers to initialise/add to, so there's no need for this syntax.`, this.node, this.isPartitionedReason);
        }
        ...
        if (this.isMapping) {
            ...
            for (const [, mappingKey] of mappingKeys) {
                if (!mappingKey.isNullifiable() && this.isWhole)
                    throw new ZKPError(`All whole states must be nullifiable, otherwise they are useless after initialisation! Consider making ${this.name} editable or constant.`, this.node);
            }
            return;
        }
    ...
    inferOwnership() {
        ...
        if (msgSenderEverywhereMappingKey && !msgSenderEverywhereMappingValue) {
            ...
            // pass the msg.sender node to the updateOwnership method
            const owner = this.nullifyingPaths[0].parent.rightHandSide ||
    ...
    ownerSetToZeroCheck() {
            ...
            for (const [, mappingKey] of mappingKeys) {
                ...
                if (keyNode.nodeType === 'FunctionCall' &&
                    ...
                    if (this.reinitialisable) {
                        const burnPath = mappingKey.keyPath.getAncestorOfType('ExpressionStatement');
        ...
        if (this.reinitialisable && !this.isBurned)
            throw new SyntaxUsageError(`The state ${this.name} has been marked as reinitialisable but we can't find anywhere to burn a commitment ready for reinitialisation.`, this.node);
    }
    searchModifyingPathsForZero() {
        var _a, _b;
        for (const path of this.modifyingPaths) {
            const assignmentNode = (_a = path.getAncestorOfType('Assignment')) === null || _a === void 0 ? void 0 : _a.node;
            if (!assignmentNode)
                continue;
    ...
    ownerSetToZeroWarning(node = this.node) {
        if (!this.reinitialisable) {
            logger.warn(`This line resets the public key inside the commitment (i.e. the owner) to 0. This means you relinquish ownership of the state and it can never be used again. \nIf you want the state to be burned and reset, mark the line where it is initialised as reinitialisable. Without doing so, you end up with a secret state of no owner which nobody can access.`);
            backtrace.getSourceCode(node.src);
        }
        else {
            logger.debug(`Found a statement which burns the secret state and allows it to be reinitialised. If this line isn't meant to do that, check why you are setting the address to 0.`);
